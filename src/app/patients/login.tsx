...
import { str } from '../../../shared/utils/ReactUtils';
...
const verifyOtp = (state: State, dispatch: React.Dispatch<Action>, updateTokenCB: (token: string) => void) => {
  ...
  Api.create(url, payload, (response) => handleVerifyOtpSuccess(updateTokenCB, response), () => handleErrorCB(dispatch));
};
...
const getOtp = (state: State, dispatch: React.Dispatch<Action>) => {
  ...
  Api.create(url, payload, () => handleGetOtpSuccess(dispatch), () => handleErrorCB(dispatch));
};
...
const PatientLogin: React.FC<{ updateTokenCB: (token: string) => void }> = ({ updateTokenCB }) => {
    ...
    <div className="md:flex items-center h-full">
      <div className="relative py-6 md:w-1/2">
        <div className="relative pb-1/2">
          <img className="absolute h-full w-full object-cover rounded-lg shadow" src="/images/login.png" alt="Login" />
        </div>
      </div>
      <div className="max-w-sm mx-auto md:w-1/2">
        <div className="md:px-6 md:pt-4">
          <div className="font-bold text-xl text-gray-700 text-center mt-6">{str('Patient Login')}</div>
          {state.ui === 'Login' ? (
            <p className="text-gray-600 text-base text-center mt-2">
              {str('We will send you an ')}
              <strong>{str('One Time Password ')}</strong>
              {str('on this mobile number')}
            </p>
          ) : (
            <p className="text-gray-600 text-base text-center mt-2">
              {str('We have sent you a ')}
              <strong>{str('One Time Password ')}</strong>
