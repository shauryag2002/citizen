...
import { Api, Routes, SkeletonLoading } from '../../../shared/utils';
...
const getPatientsList = (setState: React.Dispatch<React.SetStateAction<State>>, token: string) => {
  ...
  Api.getWithToken(Routes.url('otp/patient/'), token, (response) => handleSuccessCB(setState, response), handleErrorCB);
};
...
const showPatientCard = (patient: PatientInfo, setState: React.Dispatch<React.SetStateAction<State>>) => (
  <div
    ...
    onClick={() => setState((state) => ({ ...state, ui: 'ShowPatient', selectedPatient: patient }))}
  >
    <div className="w-full md:w-3/4">
      <div className="block text-sm md:pr-2">
        <span className="ml-1 font-semibold text-base">
          {str(`${PatientInfo.name(patient)} - ${PatientInfo.age(patient)}`)}
        </span>
      </div>
      <div className="mt-1 ml-px text-xs text-gray-900">
        <span className="ml-1">
          {str(`Last updated on ${PatientInfo.modifiedDate(patient).format('MMMM d, yyyy')}`)}
        </span>
      </div>
    </div>
...
const PatientList: React.FC<{ token: string }> = ({ token }) => {
  ...
  return (
    <div className="max-w-3xl mx-auto h-full">
      ...
      {state.ui === 'Loading' ? (
        ...
        <Patient__ShowPatient token={token} patientInfo={state.selectedPatient} />
      )}
    </div>
  );
};
